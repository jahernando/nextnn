import xyimg.utils    as ut
import xyimg.cnn      as cnn
import xyimg.dataprep as dp
import os
import argparse

path  = os.environ["LPRDATADIR"]
ipath = path+'imgs/'
opath = path+'cnn/'

# Images
pressure  = '20bar'
label     = 'xy_E_sum', 'yz_E_sum', 'zx_E_sum'
width     =  5
frames     = {'5bar' : 155, '13bar' : 50, '20bar' : 35}

# CNN configuration
cnnname   = 'HCNN'
expansion =  4 
nepochs   = 20
eloss     = 'CrossEntropyLoss'
lrate     = 0.001

parser = argparse.ArgumentParser(description='cnn')

parser.add_argument('-cnnname', type = str, help = 'name of the cnn', default = cnnname)
parser.add_argument('-tag', type = str, help = 'tag cnn name', default = '')

parser.add_argument('-pressure', type = str, help = "pressure, i.e '13bar'", default = pressure)
parser.add_argument('-label', metavar = 'N', type = str, nargs='+',
                    help = "list of images, i.e 'xy_E_sum' ", default= label)
parser.add_argument('-width', type = float, help = 'image pixel width ', default = width)

parser.add_argument('-expansion', type = int, help = "color expansion ", default = expansion)
parser.add_argument('-nepochs', type = int, help = "number of epochs", default = nepochs)
parser.add_argument('-eloss', type = str, help = 'energy loss function', default = eloss)
parser.add_argument('-lrate', type = float, help = 'learning rate', default = lrate)

args = parser.parse_args()

#--- Run

print('path : ', path)
print('args : ', args)

config                   = cnn.config
config['pressure']       = args.pressure
config['label']          = args.label
config['width']          = args.width

config['cnnname']        = args.cnnname
config['tag']            = args.tag
config['expansion']      = args.expansion
config['loss_function']  = args.eloss
config['learning_rate']  = args.lrate
config['nepochs']        = args.nepochs

print('configuration : ', config)

#-------- Input-Ouput

pressure = args.pressure
label    = args.label
width    = args.width

slabs    = ut.str_concatenate(label, '+')
swidth   = 'w'+str(int(width))
ifile    = ut.str_concatenate((pressure, slabs, swidth), '_')

cnnname  = args.cnnname
tag      = args.tag
words    = (ifile, cnnname) if tag == '' else (ifile, cnnname, tag)
ofile  = ut.str_concatenate(words, '_')

ifile = ifile + '.npz'
ofile = ofile + '.npz'
print('ifile : ', ipath + ifile)
print('ofile : ', opath + ofile)

#----- Preparation

imgdis = dp.ImgDispatchNP(ipath + ifile)
idata  = cnn.ImgDataset(imgdis)
 
CNN       = cnn.HCNN if cnnname == 'HCNN' else cnn.HKCNN if cnnname == 'HKCNN' else cnn.KCNN
padding   = 0    if cnnname == 'HCNN' else 1

shape = imgdis.x.shape
print('image shape : ', shape)
print('CNN model   : ', cnnname, ', tag :', tag)
kernel = 3
expansion = args.expansion
color   = shape[1]
nwidth  = shape[2]
assert nwidth == shape[3]
print('cnn (kernel, expansion, padding)', kernel, expansion, padding)
kcnn = CNN(color, nwidth, expansion = expansion, kernel = kernel, padding = padding)

#--- Run

nepochs = args.nepochs
print('run cnn (epochs) ', nepochs)
rcnn = cnn.run(idata, kcnn, ofilename = opath + ofile, nepochs = nepochs, config = config)

print('done!')
